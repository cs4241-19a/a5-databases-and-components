...
function createTable(data) {
  ...
  for (var x = 0; x < data.length; x++) {
    ...
    cell5.innerHTML = '<button type="button" class="btn btn-primary" onclick="editOrder(' + ordernum + ')">Edit</button>';
    cell6.innerHTML = '<button type="button" class="btn btn-primary" onclick="removeOrder(' + ordernum + ')">Remove</button>';
  }
}
...
function removeOrder(x) {
  console.log('Removing Ordernum: ' + x)

  const json = { ordernum: x },
    body = JSON.stringify(json)
...
function editOrder(x) {
  console.log('Editing Ordernum: ' + x)
  ...
  fetch('/orders')
    ...
    .then(data => {
      for (var i = 0; i < data.length; i++) {
        if (data[i].ordernum === x) {
          yourname.value = (data[i].yourname)
          phone.value = (data[i].phone)
          potato.value = (data[i].potato)
          seasoning.value = (data[i].seasoning)
          size.value = (data[i].size)
          currentordernum = (data[i].ordernum)
