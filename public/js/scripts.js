...
function createLoginForm() {
  var html =
    '<form action="" class="loginForm" id="loginForm" style="display : inline">';
  ...
  html +=
    '<button type="button" onclick="doLogin()"id="loginUser">Log In</button></form>';
...
function createGiven(elementID) {
  switch (elementID) {
    ...
    case "viewAddData":
      ...
      html +=
        '<input name="fName"type="text" id="fName" value="First Name"><br>';
      html += '<label for="lName">Last Name</label>';
      html +=
        '<input name="lName"type="text" id="lName" value="Last Name"><br>';
      html += '<label for="month">Month</label>';
      html += monthToHTML("January");
      html += daysToHTML("January", 1);
      html += '<div id="buttonDiv">';
      html +=
        '<button type="button" onclick="addData()"id="submitAddData">Submit</button>';
      html +=
        '<button type="button" onclick="cancelChange()"id="cancel">Cancel</button></div>';
      html += "</form>";
    ...
    case "viewDataMenu":
      fetch("/allDataForUser", {
        ...
        .then(function(fin) {
          ...
          html +=
            '<div id="buttonDiv"><button onclick="toggleGiven(\'viewAddData\')" type="button"id="addEntry">Add Entry</button>';
          html +=
            '<button onclick="toggleGiven(\'viewModifyData\')"type="button"id="modifyEntry">Modify Entry</button>';
          html +=
            '<button onclick="removeData()"type="button"id="deleteEntry">Delete Entry</button></div>';
          html += "</form>";
          var formDiv = document.createElement("div");
          formDiv.innerHTML = html;
          document.body.appendChild(formDiv);
      ...
      break;
    case "viewModifyData":
      var originalData = JSON.parse(
        document.getElementById(
          document.getElementById("viewDataMenuDropdown").selectedIndex
        ).innerHTML
      );
      emptyBody();
    ...
    case "viewUserMenu":
      fetch("/allUsers", {
        ...
        .then(function(fin) {
          ...
          html +=
            '<div id="buttonDiv"><button onclick="toggleGiven(\'viewAddUser\')" type="button"id="addUser">Add User</button>';
          html +=
            '<button onclick="toggleGiven(\'viewModifyUser\')"type="button"id="modifyUser">Modify User</button>';
          html +=
            '<button onclick="removeUser()" type="button"id="deleteUser">Delete User</button></div>';
          html += "</form>";
          var formDiv = document.createElement("div");
          formDiv.innerHTML = html;
          document.body.appendChild(formDiv);
    ...
    case "viewAddUser":
      ...
      html += '<div id="buttonDiv">';
      html +=
        '<button type="button" onclick="addUser()"id="submitAddUser">Submit</button>';
      html +=
        '<button type="button" onclick="cancelChange()"id="cancel">Cancel</button></div>';
      html += "</form>";
      var formDiv = document.createElement("div");
    ...
    case "viewModifyUser":
      var originalData = JSON.parse(
        document.getElementById(
          document.getElementById("viewUserMenuDropdown").selectedIndex
        ).innerHTML
      );
      emptyBody();
...
function emptyBody() {
  ...
  var filtered = [].filter.call(children, function(node) {
...
function generateHoroscope() {
  ...
  fetch("/horoscopeData", {
    ...
    .then(function(final) {
      ...
      f = final.verb[vals[5]];
      var html =
        "Friends are more sold on a " +
        a +
        " prospect than you are, which is great because you could sure use the re-assurance. Don't be discouraged by a loved one's " +
        b +
        " response.  Persevere because what you have to say needs to be heard.You could sure use someone in your " +
        c +
        " however it's not a yes-man you need, but someone who will challenge your most cherished notions. Don't let " +
        d +
        " talk you out of what you feel is " +
        e +
        ".  You know they mean well, but their advice is based on what's best for them and not for you. It took a while to " +
        f +
