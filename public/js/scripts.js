...
const getTableData = function() {
    ...
    .then(data => {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var ind = (data[i].index)
            var user = (data[i].username)

            obj = {index: ind, amount: amt, category: cat, month: mon, total: tot, username: user}
            tableData.push(obj)
            console.log(obj)
        }
...
//when a row is modified, sync changes with servers
function modifyRow(cell) {
    var row = cell.getRow()
    var index = row._row.data.index
    var category = row._row.data.category
    var amount = row._row.data.amount
    var month = row._row.data.month
    var json = {index: index, amount: amount, category: category, month: month }
    var body = JSON.stringify( json )

    console.log(json)

    console.log("modifying row at index " + index + " from server...")
    fetch('/modifyRow', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body
...
signout.onclick = signoutFunk
}


const signoutFunk = function() {
    fetch('/signout', {
        method: 'POST',
    })
        .then(function (response) { 
        // switch to login pageSizes
...
var table = new Tabulator("#expense-table", {
    ...
    columns:[   
        {title: "ID", field: "id", visible: false},               //define the table columns
        {title:"Month", field:"month", headerFilter: "input", editor:"input", cellEdited:function(cell){
            console.log(cell)
            modifyRow(cell);}},
        {title:"Category", field:"category", align:"left", headerFilter: "input", editor: "input", cellEdited:function(cell){
            console.log(cell)
            modifyRow(cell);}},
        {title:"Raw Amount", field:"amount", formatter:"money", editor: "input", bottomCalc:"sum", bottomCalcFormatter: "money", headerFilter: "input", 
